...
public class JMapViewer extends JPanel implements TileLoaderListener {
    ...
    public Point getMapPositionNoBounds(double  lat, double lon){
        int x = OsmMercator.LonToX(lon, zoom);
        int y = OsmMercator.LatToY(lat, zoom);
        x -= center.x - getWidth() / 2;
        y -= center.y - getHeight() / 2;
    ...
    protected void paintComponent(Graphics g1) {
        ...
        if(mapMarkerList.isEmpty() || mapMarkerList.size()<2) return;


        MapMarker onmMapMarker = mapMarkerList.get(0);
        for (MapMarker marker : mapMarkerList){

            if(marker.equals(onmMapMarker)) continue;
            if(marker instanceof MapMarkerDot) continue;
            if(marker instanceof MapMarkerArrow) continue;
            Point p1 = getMapPositionNoBounds(marker.getLat(), marker.getLon());
            Point p2 = getMapPositionNoBounds(onmMapMarker.getLat(), onmMapMarker.getLon());
